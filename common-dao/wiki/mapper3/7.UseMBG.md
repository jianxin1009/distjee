...
插件代码在`net.distjee.common.dao.mapper.generator`包下面，一共有如下两个类：
...
另外本项目为了在Maven中配置更简单，从1.0.0版本开始上传Maven中央仓库。  
	...
	<dependency>
	    <groupId>net.distjee.common.dao</groupId>
...
下面是一个[`generatorConfig.xml`](http://git.oschina.net/free/Mapper/blob/master/src/test/resources/generator/generatorConfig.xml)的例子：  
	...
	<generatorConfiguration>
	  <context id="Mysql" targetRuntime="MyBatis3Simple" defaultModelType="flat">
	    ...
	    <plugin type="net.distjee.common.dao.mapper.generator.MapperPlugin">
	      <property name="mappers" value="net.distjee.common.dao.mapper.common.Mapper"/>
...
看下面这段，这段代码配置了通用Mapper的插件`net.distjee.common.dao.mapper.generator.MapperPlugin`，该插件有一个必选的`<property>`属性`mappers`，这里的`mappers`和通用Mapper中的`mappers`配置含义一样，就是我们使用的通用Mapper接口，多个通用Mapper接口可以用逗号隔开。
...
先看Maven的[pom.xml](https://github.com/abel533/Mybatis-Spring/blob/spring4/pom.xml)文件(只显示有关的部分内容):  
	...
	<properties>
		...
		<targetMapperPackage>net.distjee.common.dao.mapper.mapper</targetMapperPackage>
		<targetModelPackage>net.distjee.common.dao.mapper.model</targetModelPackage>
		<!--  XML生成路径  -->
...
可以看到这个配置文件中的大多数属性都使用`${}`形式替代了。使用`<properties resource="config.properties"/>`引入了`config.properties`属性配置，该文件内容如下：  
	...
	# 通用Mapper配置
	mapper.plugin = net.distjee.common.dao.mapper.generator.MapperPlugin
	mapper.Mapper = net.distjee.common.dao.mapper.common.Mapper
...
###三、Mapper.xml文件[`UserInfoMapper.xml`](https://github.com/abel533/Mybatis-Spring/blob/spring4/src/main/resources/mapper/UserInfoMapper.xml)  
	...
	<mapper namespace="net.distjee.common.dao.mapper.mapper.UserInfoMapper" >
		<resultMap id="BaseResultMap" type="net.distjee.common.dao.mapper.model.UserInfo" >
			<!--
