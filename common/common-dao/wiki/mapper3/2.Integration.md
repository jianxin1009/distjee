...
<b>你没看错，就是这么配置的，注意这里是`net.distjee.common.dao.xxx`,和MyBatis的唯一区别就是`org.`改成了`tk.`，方便修改和记忆。

通用Mapper的各项属性通过`properties`属性进行配置，如果默认配置就是一行`mappers=net.distjee.common.dao.mapper.common.Mapper`时，可以不写，就会变成：
